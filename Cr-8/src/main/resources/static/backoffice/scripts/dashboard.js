document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all([fetch(API_INFO_URL).then(res => res.json()), fetch(API_BOOKINGS_URL).then(res => res.json())])
  ...
  function processAllInfoData(data) {
    const completedCount = data.filter(item => item.status && item.status.name === "Completed").length;
    const pendingCount = data.filter(item => item.status && item.status.name === "Pending").length;
  ...
  function processAllBookingsData(data) {
    const completedCount = data.filter(item => item.status && item.status.name === "Completed").length;
    const pendingCount = data.filter(item => item.status && item.status.name === "Pending").length;
    const cancelledCount = data.filter(item => item.status && item.status.name === "Cancelled").length;
    ...
    createCard(
      ...
      ["#fc931c", "#ed008a", "#ffff22"] // Orange, Purple, Yellow colors
    );
  }

  // Function to create a card with a pie chart
  function createCard(title, completedCount, pendingCount, cancelledCount, labels, colors) {
        ...
        <div class="card">
          <div class="card-body">
            ...
            <p class="text-center">
              ...
              ${cancelledCount > 0 ? ` | <span class="text-orange">Cancelled: ${cancelledCount}</span>` : ""}
            </p>
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            data: [completedCount, pendingCount, cancelledCount].filter(value => value > 0),
  ...
  function initializeCalendar(bookedDates) {
    if (typeof FullCalendar === "undefined") {
      console.error("FullCalendar non è definito. Verifica che sia stato caricato correttamente.");
      return;
    }
    ...
    const events = bookedDates.map(booking => ({
      ...
      start: booking.date,
      end: booking.toDate ? new Date(new Date(booking.toDate).getTime() + 24 * 60 * 60 * 1000).toISOString().split("T")[0] : null,
      allDay: true,
